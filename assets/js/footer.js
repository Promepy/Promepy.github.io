// ============================================
// Shared Footer Component
// ============================================
(function () {
    'use strict';

    function getBasePath() {
        const path = window.location.pathname;
        if (path.includes('/blog/') || path.includes('/projects/')) {
            return '../';
        }
        return '';
    }

    function createFooter() {
        const base = getBasePath();
        const year = new Date().getFullYear();

        const footer = document.createElement('footer');
        footer.className = 'site-footer';
        footer.setAttribute('role', 'contentinfo');

        footer.innerHTML = `
      <div class="container">
        <div class="footer-grid">
          <div class="footer-col">
            <h4>Gowtham M.</h4>
            <p>Python AI & Automation Specialist. I help businesses automate workflows and integrate AI to save time and scale.</p>
            <div class="footer-social">
              <a href="https://github.com/Promepy" target="_blank" rel="noopener" aria-label="GitHub">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M15 22v-4a4.8 4.8 0 0 0-1-3.5c3 0 6-2 6-5.5.08-1.25-.27-2.48-1-3.5.28-1.15.28-2.35 0-3.5 0 0-1 0-3 1.5-2.64-.5-5.36-.5-8 0C6 2 5 2 5 2c-.3 1.15-.3 2.35 0 3.5A5.403 5.403 0 0 0 4 9c0 3.5 3 5.5 6 5.5-.39.49-.68 1.05-.85 1.65-.17.6-.22 1.23-.15 1.85v4"/><path d="M9 18c-4.51 2-5-2-7-2"/></svg>
              </a>
              <a href="#" target="_blank" rel="noopener" aria-label="LinkedIn">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M16 8a6 6 0 0 1 6 6v7h-4v-7a2 2 0 0 0-2-2 2 2 0 0 0-2 2v7h-4v-7a6 6 0 0 1 6-6Z"/><rect x="2" y="9" width="4" height="12"/><circle cx="4" cy="4" r="2"/></svg>
              </a>
              <a href="https://www.youtube.com/@gowthammuraharirao" target="_blank" rel="noopener" aria-label="YouTube">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M2.5 17a24.12 24.12 0 0 1 0-10 2 2 0 0 1 1.4-1.4 49.56 49.56 0 0 1 16.2 0A2 2 0 0 1 21.5 7a24.12 24.12 0 0 1 0 10 2 2 0 0 1-1.4 1.4 49.55 49.55 0 0 1-16.2 0A2 2 0 0 1 2.5 17"/><path d="m10 15 5-3-5-3z"/></svg>
              </a>
            </div>
          </div>
          <div class="footer-col">
            <h4>Pages</h4>
            <a href="${base}index.html">Home</a>
            <a href="${base}projects.html">Projects</a>
            <a href="${base}blog/index.html">Blog</a>
            <a href="${base}youtube.html">YouTube</a>
            <a href="${base}about.html">About</a>
          </div>
          <div class="footer-col">
            <h4>Services</h4>
            <a href="${base}index.html#services">Data Extraction & ETL</a>
            <a href="${base}index.html#services">AI & LLM Solutions</a>
            <a href="${base}index.html#services">Business Automation</a>
          </div>
          <div class="footer-col">
            <h4>Connect</h4>
            <a href="${base}contact.html">Contact</a>
            <a href="${base}community.html">Community</a>
            <a href="https://www.youtube.com/@gowthammuraharirao" target="_blank" rel="noopener">YouTube Channel</a>
          </div>
        </div>
        <div class="footer-bottom">
          <p>&copy; ${year} Gowtham M. All rights reserved.</p>
          <p><a href="${base}terms.html">Terms</a> &nbsp;·&nbsp; <a href="${base}privacy.html">Privacy</a></p>
        </div>
      </div>
    `;

        document.body.appendChild(footer);
    }

    document.addEventListener('DOMContentLoaded', createFooter);
})();
